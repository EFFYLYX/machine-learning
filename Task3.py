...
# the class to store the information of a node in a decison tree
class node:
...
def information_gain(data_set, attribute_index):
    ...
    # compute the sum of entropy for subset S_u_i where A = u_i
...
# to test the program, you can change the csv_data_name, drop_column_name = 'Day', target_name

csv_data_name = 'Tennis.csv'
drop_column_name = 'Day'    # drop the column that does not need to be covered
target_name = 'PlayTennis'

##################################################
# read csv data for Tennis data set
csv_data = pd.read_csv(csv_data_name)
data = csv_data.drop(columns=[drop_column_name])
data = data.to_dict('index')

# rearrange the data set for later processing
(data_set, attributes) = rearrange_data(data, target_name)

list_index_attributes = []
for i in range(0, len(attributes)):
    list_index_attributes.append(i)

print('==============================')

print('Generating Decision Tree...')
t = id3(data_set, list_index_attributes)
